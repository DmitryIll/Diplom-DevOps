...
- создает YDB и табилце в ней для реализации блокировки tfstae
- мигрируем tfstate.

Код проекта см. в папке init_project.
...
В процессе создания access_key и secret_key и API к БД, правлю конфиг авторизации к БД.
Далее перемещаю tfstate в s3 для этого раскоментирую блок backend "s3" в providers.tf и выполняю:

![alt text](image-1.png)

Итого - создан сервисный аккаунт для работы terraform, создан S3 хранилище, создана БД YDB с таблицей блокировки - Dynamo DB Table, в S3 хранилище перемещен tfstate для иницилизирующего проекта.
...
VPC с разными зонами доступности буду создавать уже в отдельном проекте, т.к. это относится уже к проекту и структуре решения.
