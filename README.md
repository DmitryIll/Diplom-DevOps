...
Дополню цели чтобы исходя из требований более точно определить требования к реализации решения.

Кейс:
Меня пригласили на проект техлидом DevOps инженеров в компанию "D-org" для целей:
- Получить отказоустойчивую работу сайта компании в яндекс облаке.
...
- Создать отдельные облака:
-- "prod" - для работы штатной инфраструктуры - сайта.
-- "stage" - для тестового стенда для выполнения приемочных тестов работы новой версии сайта.

У компании уже есть группа разработчиков, которые, на своих ПК разрабатывают сайт компании. Разработчики "на выходе" выдают статичные файлы html для веб сайта.
- Обеспечить инфраструктуру - git репозиторий, таск-менеджер + Wiki для работы команды и фиксации решений, инструкций и др. 
...
Отказоустойчивость:
- Обеспечить отказоустойчивость "в проде" - сайт должен работать в кластере kubernetis на трех рабочих нодах, расположенных в разных датацентрах.
- Отказоустойчивость в "stage" не требуется - развертывать кубернетис без отказоустойчивости - для экономии ресурсов организации.
- Обеспечить автоматизацию - при выкладке новой версии сайта, должна срабатывать автоматизация - сайт должен развертываться в облаке автоматически в порядке:
1 - поднятие сайта в отдельном облаке - "stage" - облако для приемки новой версии - выполнения автотестов и ручных тестов. 
2 - В случае если к новой версии нет замечаний, то, при нажатии "одной кнопки" в системе автоматиации - должна удалиться инфраструктура тествого стенда из "stage" облака (для экономии ресурсов), и автоматическое должно быть развертывание (обновление) новой вервии в "проде" в отдельном "штатном" облаке - "prod".

- Развертывание инфраструктуры и управление должны быть через ПО Terraform. При этом terraform-state файл должен находится в облаке в S3 хранилище + обеспечивать работу блокировки tfstate-backend, если какой-либо сотрудник вносит изменения в tfstate, + обеспечить версионирование (для возможности отката), + реализовать резервное копирование.

Проектирование тербований к CI-CD:
- в качестве Git репозитория и системы "CI-CD" - использовать сервис gitlab.com - для минимизации затрат на создание и поддержку собственноо сервера.
- В gitlab организовать группу репозиториев для проекта, дать доступ для разработчиков для выкладки файлов сайта.
- Настроить CI-CD автоматизацияю для развертывания сайта в "stage" и "prod" .

Облака:
- для "prod" и "stage" сред - создать  отдельные облака "prod" и "stage" - чтобы полностью изолировать штатную среду от тестовой, минимизировать риск сбоев штатной среды от ошибок работы в тестовой среде.
- Создание облаков "prod" и "stage" - должно быть через IaC подход и через ПО terraform, для этого использовать отдельное облако "cloud-adm" - применять в этом облаке код создания и управления облаками. 
